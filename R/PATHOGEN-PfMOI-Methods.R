...
init_MICRO_PfMOI <- function(PfMOI = NULL, tStart = 0){
  ...
  private$PfID = 1L
  if(is.null(private$Pathogens$Pf)){ # only add the PfMOI object if NULL
    self$set_humanPfMOI()
  } else {
    if(length(PfMOI)!=self$nHumans){
...
Human_set_humanPfMOI <- function(PfID, tInf = -1L, MOI = 0L, b = 0.55, c = 0.15, damID = -1L, sireID = -1L, chemoprophylaxis = FALSE){
  private$Pathogens = humanPfMOIcpp(PfID, tInf, MOI, b, c, damID, sireID, chemoprophylaxis)
}
